...
vector <float> dot (const vector <float>& m1, const vector <float>& m2, const int m1_rows, const int m1_columns, const int m2_columns) {
    ...
    // not block index.
    // Note the extra padding on the bounds of the block loops.
    for (int block_y = 0; block_y < m1_rows + block_size; block_y += block_size) {
        for (int block_x = 0; block_x < m2_columns + block_size; block_x += block_size) {
            // Inside of block
            for( int row = block_y; row < min(block_y + block_size, m1_rows); ++row ) {
                for( int col = block_x; col < min(block_x + block_size, m2_columns); ++col ) {
